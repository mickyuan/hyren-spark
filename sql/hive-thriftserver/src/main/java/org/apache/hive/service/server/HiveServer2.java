...
public class HiveServer2 extends CompositeService {
  ...
  private void addServerInstanceToZooKeeper(HiveConf hiveConf) throws Exception {
    ...
    try {
      ...
      /*
      instanceURI node01:32500
       */
      LOG.info("znodeData###################"+instanceURI.split(":").length);
      for (int i = 0; i < instanceURI.split(":").length ; i++) {
        LOG.info("znodeData###################"+instanceURI.split(":")[i]);
      }
      byte[] znodeDataUTF8 = ("hive.server2.authentication=NONE;" +
              ...
              instanceURI.split(":")[1] +
              ";hive.server2.use.SSL=false").getBytes(Charset.forName("UTF-8"));
      znode =
  ...
  /*
  * Delete the starting znode node from zookeeper
  * @ BY-HLL
  * */
  ...
  private String getServerInstanceURI(HiveConf hiveConf) throws Exception {
    ...
    LOG.info(getHiveHost()+"###################"+thriftCLIService.getPortNumber());
    return getHiveHost() + ":"
            + thriftCLIService.getPortNumber();
  }
  //考虑提供的spark-thrift-server服务可能为集群外调用，且无法通过hostname连接到服务，就需要修改zookeeper中的注册信息为ip
  private String getHiveHost() {
    HiveConf hiveConf = thriftCLIService.getHiveConf();
    String hiveHost = hiveConf.getVar(ConfVars.HIVE_SERVER2_THRIFT_BIND_HOST);
  ...
  static void deleteServerInstancesFromZooKeeper(String versionNumber) throws Exception {
    ...
    String zooKeeperEnsemble = ZooKeeperHiveHelper.getQuorumServers(hiveConf);
    String rootNamespace = hiveConf.getVar(ConfVars.HIVE_SERVER2_ZOOKEEPER_NAMESPACE);
    int baseSleepTime = (int) hiveConf.getTimeVar(ConfVars.HIVE_ZOOKEEPER_CONNECTION_BASESLEEPTIME, TimeUnit.MILLISECONDS);
    int maxRetries = hiveConf.getIntVar(ConfVars.HIVE_ZOOKEEPER_CONNECTION_MAX_RETRIES);
